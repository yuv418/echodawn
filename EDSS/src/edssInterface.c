...
char rtpAddressNoListen[61]; // maximum ip:port length
...
EDSS_STATUS edssInitServer(edssConfig_t *edssCfg, char **sdpBuffer) {
  ...
  snprintf(rtpAddressNoListen, sizeof(rtpAddressNoListen), "tcp://%s:%d/",
           edssCfg->socketAddr, edssCfg->port);
  snprintf(rtpAddress, sizeof(rtpAddress), "%s?listen=1", rtpAddressNoListen);

  rtpFmt = av_guess_format("mpegts", NULL, NULL);
  if (!rtpFmt) {
    EDSS_LOGE("Failed to guess format mpegts\n");
    return EDSS_LIBAV_FAILURE;
  }
  ...
  *sdpBuffer = malloc(61);
  strcpy(*sdpBuffer, rtpAddressNoListen);

  EDSS_LOGI("Copied sdpBuffer as %s\n", *sdpBuffer);
